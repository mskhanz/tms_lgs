...
@section('title', 'Take Quiz - ' . $attempt->quiz->title)
...
<div class="page-header">
    <h1><i class="bi bi-pencil-square me-2"></i>{{ $attempt->quiz->title }}</h1>
    <div class="d-flex justify-content-between align-items-center">
        ...
        <div class="d-flex align-items-center gap-3">
            @if($endsAt)
            <div class="quiz-timer" id="timer" data-ends="{{ $endsAt->timestamp }}">
...
<form method="POST" action="{{ route('trainee.quizzes.submit', $attempt) }}" id="quiz-form" class="quiz-protected-content">
    @csrf
    ...
    @if($question->part && $question->part !== $currentPart)
    @php $currentPart = $question->part; @endphp
    <div class="alert alert-light border mt-4 mb-3"><strong>{{ $currentPart }}</strong></div>
    @endif

    <div class="card question-card mb-3" data-question-id="{{ $question->id }}" id="question-{{ $question->id }}">
        <div class="card-body">
            ...
            <label class="option-label d-flex align-items-start gap-2">
                <input type="radio"
                       ...
                       {{ (string) ($savedAnswers[$question->id] ?? '') === (string) $option->id ? 'checked' : '' }}>
...
(function () {
    ...
    const saveUrl = @json(route('trainee.quizzes.save', $attempt));
    const indexUrl = @json(route('trainee.quizzes.index'));
