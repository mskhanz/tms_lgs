...
{{-- On-screen report (includes CNIC) --}}
    ...
    <div class="table-responsive">
        <table class="table quiz-report-table mb-0">
            ...
            <tbody>
                ...
                <tr>
                    <td>{{ $index + 1 }}</td>
                    <td><strong>{{ $traineeName($attempt->user) }}</strong></td>
                    <td>{{ $traineeCnic($attempt->user) }}</td>
...
{{-- Print uses the same document layout as PDF (no CNIC) --}}
<div class="quiz-print-document print-only">
    @include('admin.quizzes._results-document', [
