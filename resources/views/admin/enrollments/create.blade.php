...
<div class="row">
    <div class="col-lg-8">
        ...
        <div class="card">
            ...
            <div class="card-body">
                <form method="POST" action="{{ route('admin.enrollments.store') }}" id="enrollmentForm">
                    @csrf
                        ...
                        <select name="trainee_ids[]" id="trainee_ids" class="form-select" multiple required>
                            @forelse($trainees as $trainee)
                                <option value="{{ $trainee->id }}"
                                        ...
                                        data-cnic="{{ $trainee->traineeProfile->cnic_no ?? 'N/A' }}"
                                        data-designation="{{ $trainee->traineeProfile->designation ?? 'N/A' }}"
                                        data-photo="{{ $trainee->photoUrl() }}"
                                        data-initial="{{ strtoupper(substr($trainee->name, 0, 1)) }}"
                                        {{ in_array($trainee->id, old('trainee_ids', [])) ? 'selected' : '' }}>
                                    {{ $trainee->name }} - {{ $trainee->traineeProfile->cnic_no ?? $trainee->email }}
                                </option>
                            @empty
                        ...
                        <select name="training_batch_id" id="training_batch_id" class="form-select" required>
                            ...
                            @foreach($batches as $batch)
                                <option value="{{ $batch->id }}" 
                                        data-program="{{ $batch->trainingProgram->title }}"
                                        data-code="{{ $batch->batch_code }}"
                                        data-start="{{ \Carbon\Carbon::parse($batch->start_date)->format('d M, Y') }}"
                                        data-end="{{ \Carbon\Carbon::parse($batch->end_date)->format('d M, Y') }}"
                                        data-seats="{{ $batch->total_seats }}"
                                        data-filled="{{ $batch->seats_filled }}"
                                        data-available="{{ $batch->total_seats - $batch->seats_filled }}"
                                        {{ old('training_batch_id') == $batch->id ? 'selected' : '' }}>
                                    {{ $batch->trainingProgram->title }} - {{ $batch->batch_code }}
                                    ({{ $batch->total_seats - $batch->seats_filled }} seats available)
                                </option>
                            @endforeach
                        </select>
    ...
    <div class="col-lg-4">
        ...
        <div class="card mt-3">
            ...
            <div class="card-body">
                <div class="d-flex justify-content-between mb-2">
                    ...
                    <span class="fw-semibold">{{ $enrolledTrainees->count() }}</span>
                </div>
                <div class="d-flex justify-content-between mb-2">
                    <span class="text-muted small">Unenrolled trainees:</span>
                    <span class="fw-semibold">{{ $trainees->count() }}</span>
                </div>
                <div class="d-flex justify-content-between mb-2">
                    <span class="text-muted small">Active Batches:</span>
                    <span class="fw-semibold">{{ $batches->count() }}</span>
                </div>
                <div class="d-flex justify-content-between">
                    <span class="text-muted small">Total Seats:</span>
                    <span class="fw-semibold">{{ $batches->sum('total_seats') }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="card mt-3">
            ...
            <div class="card-body p-0">
                @forelse($enrolledTrainees as $enrolledTrainee)
                    @php $currentEnrollment = $enrolledTrainee->enrollments->first(); @endphp
                    <div class="d-flex align-items-center gap-2 px-3 py-2 {{ ! $loop->last ? 'border-bottom' : '' }}">
                        @if($enrolledTrainee->photoUrl())
                            <img src="{{ $enrolledTrainee->photoUrl() }}" alt="{{ $enrolledTrainee->name }}" class="trainee-info-photo" style="width: 36px; height: 36px;">
                        @else
                            <span class="trainee-info-fallback" style="width: 36px; height: 36px; font-size: 0.85rem;">{{ strtoupper(substr($enrolledTrainee->name, 0, 1)) }}</span>
                        @endif
                        <div class="flex-grow-1 min-w-0">
                            <div class="fw-semibold small text-truncate">{{ $enrolledTrainee->name }}</div>
                            <div class="text-muted" style="font-size: 0.75rem;">
                                {{ $currentEnrollment?->trainingBatch?->trainingProgram?->title ?? 'N/A' }}
                                @if($currentEnrollment?->trainingBatch?->batch_code)
                                    · {{ $currentEnrollment->trainingBatch->batch_code }}
                                @endif
                            </div>
                        </div>
                    </div>
...
$(function () {
    ...
    const traineePhotos = @json($trainees->mapWithKeys(fn ($trainee) => [(string) $trainee->id => $trainee->photoUrl()]));
    const traineeInitials = @json($trainees->mapWithKeys(fn ($trainee) => [(string) $trainee->id => strtoupper(substr($trainee->name, 0, 1))]));
