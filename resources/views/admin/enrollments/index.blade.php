...
<div class="card">
    <div class="card-body">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="d-flex align-items-center">
                                <div class="user-avatar-small me-2">
                                    @if($enrollment->trainee->photoUrl())
                                        <img src="{{ $enrollment->trainee->photoUrl() }}"
                                             alt="{{ $enrollment->trainee->name }}"
                                ...
                                <div>
                                    <div class="fw-semibold">{{ $enrollment->trainee->name }}</div>
                                    <small class="text-muted">{{ $enrollment->trainee->email }}</small>
                                </div>
                            </div>
                        </td>
                        <td>
                            @if($enrollment->trainee->traineeProfile)
                                {{ $enrollment->trainee->traineeProfile->cnic_no ?? 'N/A' }}
                            @else
                                <span class="text-muted">N/A</span>
                            @endif
                        </td>
                        <td>
                            <div class="fw-semibold">{{ $enrollment->trainingBatch->trainingProgram->title }}</div>
                            <small class="text-muted">{{ $enrollment->trainingBatch->trainingProgram->code }}</small>
                        </td>
                        <td>
                            <span class="badge bg-secondary">{{ $enrollment->trainingBatch->batch_code }}</span>
                        </td>
                        ...
                        <td>
                            ...
                            @endphp
                            <span class="badge bg-{{ $color }}">{{ ucwords(str_replace('_', ' ', $enrollment->status)) }}</span>
                        </td>
                        <td>
                            ...
                            <!-- Status Update Modal -->
                            <div class="modal fade" id="statusModal{{ $enrollment->id }}" tabindex="-1">
                                        ...
                                        <form method="POST" action="{{ route('admin.enrollments.update-status', $enrollment->id) }}">
                                            ...
                                            <div class="modal-body">
                                                ...
                                                <div class="mb-3">
                                                    ...
                                                    <input type="date" name="completion_date" class="form-control" value="{{ $enrollment->completion_date }}">
                                                </div>
                                                <div class="mb-3">
                    ...
                    <tr>
                        <td colspan="8" class="text-center py-5">
                            ...
                            <a href="{{ route('admin.enrollments.create') }}" class="btn btn-primary btn-sm mt-3">
        ...
        @include('admin.partials.pagination', ['paginator' => $enrollments, 'label' => 'enrollments'])
    </div>
</div>
